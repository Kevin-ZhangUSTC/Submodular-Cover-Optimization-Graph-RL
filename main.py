...
The script will:
  ...
  2. Run the greedy baseline and capture its trajectory for warm-start.
  3. Pre-train the GNN policy via imitation learning (behavioural cloning).
  4. Fine-tune with REINFORCE + Actor-Critic baseline (with optional
     blended imitation annealing).
  5. Save the trained model to ``checkpoint.pt``.
  6. Plot the training curves to ``training_curve.png``.
"""
